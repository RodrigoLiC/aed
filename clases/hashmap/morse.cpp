...
int countUnique(string* words, int size) {
    Hashmap<string, string> hash(16, 0.75, 4);
    int count = 0;
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (hash.search(morse) == "") {
            hash.insert(morse, word);
            //cout << "inserted: " << morse << " " << word << endl;
            count++;
        }
        else {
            hash.insert(morse, hash.search(morse) + " " + word);
        }
    }
    //hash.print();
...
int main() {
    ...
    string test2[] = {"a"};
    string test3[] = {"abc", "def", "ghi", "jkl"};

    wordsPtr = test1;
    countUnique(wordsPtr, 4);

    wordsPtr = test2;
    countUnique(wordsPtr, 1);
